...
namespace GreenOnions.ChatGPTClient
{
    public class ChatClient : IPlugin
    {
        ...
        private ConcurrentDictionary<long, TimeOutClass> ChatingUser = new ConcurrentDictionary<long, TimeOutClass>();
        ...
        public void OnConnected(long selfId, IGreenOnionsApi api)
        {
            ...
            else
            {
                ...
                {
                    APIkey = "",
                    StartCommands = new[] { "<机器人名称>尬聊", "<机器人名称>ChatGPT" },
                    ExitCommands = new[] { "<机器人名称>不聊了", "<机器人名称>住口", "<机器人名称>闭嘴" },
                    ChatStartMessage = "您已进入ChatGPT聊天模式，如需退出请发送\"<机器人名称>闭嘴\"",
                ...
                File.WriteAllText(Path.Combine(_pluginPath!, "说明.txt"), "使用前，用户需要自行魔法上网，注册OpenAI账号，并在 https://beta.openai.com/account/api-keys 处申请一个API-Key\r\n" +
                    ...
                    else
                    {
                        if (_startCmds != null)
                        {
                            if (_startCmds.Any(c => string.Equals(c, msg.Text, StringComparison.OrdinalIgnoreCase)))  //开始聊天
                            {
                                ...
                                ChatingUser.TryAdd(msgs.SenderId, new TimeOutClass { TimeOutAt = DateTime.Now.AddSeconds(_config.TimeOutSecond), TimeOutDo = Response });
                                return true;
                            }
                        }
                    }
        ...
        private class TimeOutClass
        {
            ...
            public Action<GreenOnionsMessages>? TimeOutDo { get; set; }
        }
    }
}