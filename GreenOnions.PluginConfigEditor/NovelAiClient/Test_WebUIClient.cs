...
namespace GreenOnions.NovelAiClient
{
    ...
    public class Test_WebUIClient
    {
        ...
        public Test_WebUIClient(string host, int fnIndex, int promptIndex, int undesiredIndex)
        {
            _host = host;
            _fnIndex = fnIndex;
        ...
        public async Task<byte[]?> PostAsync(ArrayList data)
        {
            ...
            if (jToken is null)
                throw new Exception($"API返回为空：{(int)response.StatusCode} {response.StatusCode} 请检查地址、端口是否正确，以及是否已启用API");
            if (jToken["error"] is not null)
                throw new Exception($"API返回值中含有错误：{jToken["error"]}");
